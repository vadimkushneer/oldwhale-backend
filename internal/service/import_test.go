...
func TestRequestProviderAIModelsUsesAnthropicHeaders(t *testing.T) {
	...
	defer ts.Close()
	s := &AICatalogService{httpClient: ts.Client()}
	models, err := s.requestProviderAIModels(context.Background(), adminAIModelProviderByID("anthropic"), ts.URL, "secret-key")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRequestProviderAIModelsUsesBearerHeaders(t *testing.T) {
	...
	defer ts.Close()
	s := &AICatalogService{httpClient: ts.Client()}
	models, err := s.requestProviderAIModels(context.Background(), adminAIModelProviderByID("openai"), ts.URL, "secret-key")
	if err != nil {
		t.Fatal(err)
	}
